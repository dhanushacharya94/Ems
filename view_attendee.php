...
<?php
if(isset($_GET['id'])){
	$qry = $conn->query("SELECT a.*,concat(a.firstname,' ',a.middlename,' ',a.lastname) as name,e.event FROM attendees a inner join events e on e.id = a.event_id where a.id = ".$_GET['id'])->fetch_array();
foreach($qry as $k => $v){
	$$k = $v;
}
}
