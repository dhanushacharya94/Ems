		...
		<?php else: ?>
			<table class="table table-condensed table-bordered table-hover" id="att-records">
				...
				<tbody>
					<?php 
	                $i = 1;
					$attendees = $conn->query("SELECT *,concat(firstname,' ',middlename,' ',lastname) as name FROM attendees where event_id= $eid order by concat(lastname,', ',firstname,' ',middlename) asc ");
